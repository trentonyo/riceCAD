...
let getCurrentMetaData = function ()
{
    ...
    {
        let currentLabelStyle = window.getComputedStyle(checkedLabels[i])
        let currentLabel = {
            "background-color" : currentLabelStyle.getPropertyValue("background-color"), //TODO this doesn't work
            "text-color" : currentLabelStyle.getPropertyValue("color"),
            "tag" : checkedLabels[i].value
        }
...
let postThumbnail = function (event) {
    let content = {
...
let postProject = function (event) {
    let content = {
        "plan" : getCurrentPlan(),
        "projectID" : projectID
    }
...
let postMetaData = function (event) {
