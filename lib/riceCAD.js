...
let camerasQuatRX = [-0.26, 0.06,-0.12,-0.77,-0.26]
let camerasQuatRY = [ 0.37, 0.92,-0.7 , 0.0 , 0.0 ]
let camerasQuatRZ = [ 0.10, 0.17,-0.12, 0.0 , 0.0 ]
let camerasQuatRW = [ 0.88,-0.35, 0.7 , 0.63, 0.97]

let camerasQuatPX = [ 45.0,-13.0,-23.0, 9.0 , 0.0 ]
let camerasQuatPY = [ 32.0, 14.0, 17.0,35.0 ,10.0 ]
let camerasQuatPZ = [ 45.0,-13.0, 12.0,-2.0 ,10.0 ]
...
// {
//     camera.setRotation( camerasQuatRX[cam],
//         camerasQuatRY[cam],
//         camerasQuatRZ[cam],
//         camerasQuatRW[cam],
//     )
//
//     camera.setPosition( camerasQuatPX[cam],
//         camerasQuatPY[cam],
//         camerasQuatPZ[cam])
//
...
let localEulerY = camera.getEulerAngles().y
let localEulerX = camera.getEulerAngles().x

mouse.on("mousemove", function (event) {
    // noinspection JSSuspiciousNameCombination
    if (event.buttons[pc.MOUSEBUTTON_LEFT] && !viewportLocked)
    {
        localEulerX += rotateSpeed * event.dy  //dy is added to the opposite angle because matrices I guess
        localEulerY += rotateSpeed * event.dx  //dx is added to the opposite angle because matrices I guess
...
let handleNumKeyCamera = function (event)
{
    let digit = event.key - pc.KEY_0
    if(0 <= digit && digit < 10 && !viewportLocked)
    {
        //Set a camera angle
        console.log("Euler angles:", camera.getEulerAngles())
        console.log("Local Euler angles:", camera.getLocalEulerAngles())
        console.log("Global position:", camera.getPosition())
        ...
        if (keyboard.isPressed(pc.KEY_SHIFT))
        {
            ...
            camerasQuatRX[digit] = camera.rotation.x
            camerasQuatRY[digit] = camera.rotation.y
            camerasQuatRZ[digit] = camera.rotation.z
            camerasQuatRW[digit] = camera.rotation.w

            camerasQuatPX[digit] = camera.position.x
            camerasQuatPY[digit] = camera.position.y
            camerasQuatPZ[digit] = camera.position.z

        }
        else
        {
            //Pick a camera angle
            // setCamera(digit) TODO uncomment, debugging rn
        }
    }
}
keyboard.on("keydown", handleNumKeyCamera)
