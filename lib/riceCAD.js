...
httpRequest_projectPlan.onreadystatechange = function (event)
{
    if(httpRequest_projectPlan.readyState === 4 && httpRequest_projectPlan.status === 200)
    {
        console.log("Project loaded, parsing...")
        try
        {
    ...
    {
        console.log(`Still waiting on plan request... (readyState: ${httpRequest_projectPlan.readyState}, status: ${httpRequest_projectPlan.status})`)
    }
}


//Loading project metadata
// const httpRequest_metaData = new XMLHttpRequest()
// const url_metaData = "/projectMetaData.json"
//
// let projectMetaData = false
// let currentProjectID = "xas32"
//
// let handleMetaData
//
// httpRequest_metaData.open("GET", url_metaData)
// httpRequest_metaData.send()
//
// httpRequest_metaData.onreadystatechange = function (event)
// {
//     if(httpRequest_metaData.readyState === 4 && httpRequest_metaData.status === 200)
//     {
//         console.log("Metadata loaded, JSON parsing...")
//         try
//         {
//             projectMetaData = JSON.parse(httpRequest_metaData.responseText)[currentProjectID]
//             handleMetaData(projectMetaData)
//         }
//         catch (err)
//         {
//             console.log(`--Error ${err} while parsing project metadata, HTTP response below`)
//             console.log(httpRequest_metaData.responseText)
//             console.log(httpRequest_metaData)
//             console.log("--End HTTP response")
//         }
//     }
//     else
//     {
//         console.log(`Still waiting on metadata request... (readyState: ${httpRequest_metaData.readyState}, status: ${httpRequest_metaData.status})`)
//     }
// }
//
// handleMetaData = function (meta)
// {
//     //TODO rework to take in the plan file, metadata now loaded by handlebars
//
//
//
//     // projectNameField.innerText = meta.title
//     //
//     // viewportBackground.value = meta.palette["background"].color
//     // workingPlaneColor.value = meta.palette["workingplane"].color
//     //
//     // updateViewportSettings(false)
//     //
//     // for(let i = 1; i <= 9; i++)
//     // {
//     //     material_palette[i] = new Material("Custom", meta.palette[i.toString()].color, meta.palette[i.toString()].glass)
//     // }
//     //
//     // loadFromTextArea()
// }

let generateNewProjectID = function () {
    return "STUB"
}

// Posting new project metadata
const url_postMetaData = "/projectMetaData.json"

let tmpObject = {
    "projectID" : projectID ? projectID : generateNewProjectID(),
    "metaData" :
    {
        "title" : "Server Test Title",
        "description" : "This is also a server post test",
        "tags" : [
            {
        ...
        "downloads" : 0,
        "palette" : {
            "background": {"color": "#f5ed92", "glass":  false},
            "workingplane": {"color": "#b3d5ac", "glass":  false},
            "1": {"color": "#492b22", "glass":  false},
            "2": {"color": "#67f1ff", "glass":  false},
            "3": {"color": "#c7b5da", "glass":  true},
            "4": {"color": "#59f786", "glass":  false},
            "5": {"color": "#faa926", "glass":  false},
            "6": {"color": "#f9abcd", "glass":  true},
            "7": {"color": "#b78926", "glass":  false},
            "8": {"color": "#f9faaa", "glass":  false},
            "9": {"color": "#a9f978", "glass":  false}
        }
    }
}

// axios.post(url_postMetaData, tmpObject)
